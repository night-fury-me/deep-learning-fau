...
def gradient_check_weights(layers, input_tensor, label_tensor, bias):
    ...
    layers_numerical = deepcopy(layers)
    ...
    while not it.finished:
        ...
        if bias:
            layers_numerical[0].bias = plus_epsilon
        else:
            layers_numerical[0].set_weight(plus_epsilon)

        plus_epsilon_activation = input_tensor.copy()
        for layer in layers_numerical[:-1]:
            plus_epsilon_activation = layer.forward(plus_epsilon_activation)

        if bias:
            layers_numerical[0].bias = minus_epsilon
        else:
            layers_numerical[0].set_weight(minus_epsilon)

        minus_epsilon_activation = input_tensor.copy()
        for layer in layers_numerical[:-1]:
            minus_epsilon_activation = layer.forward(minus_epsilon_activation)

        upper_error = layers_numerical[-1].forward(plus_epsilon_activation, label_tensor)
        lower_error = layers_numerical[-1].forward(minus_epsilon_activation, label_tensor)
        diff = upper_error - lower_error
        numerical_derivative = (upper_error - lower_error) / (2 * epsilon)
